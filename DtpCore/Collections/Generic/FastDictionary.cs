// Source core from Microsoft. 
// This implementations optimizes access to values as fast as possible, by enabling direct access to data arrays and by use of code inlining removes method calls.
// Performance is about twice as fast.
// Dtp changes: 
// Private fields, made internal for extension method access.
// Extensions methods made with [MethodImpl(MethodImplOptions.AggressiveInlining)] for inlining code directly into the search processing of the graph.
...
{
    public class FastIntDictionary<TValue> : FastDictionary<int, TValue>
    {

        public FastIntDictionary() : base(0, null) { }

        public FastIntDictionary(int capacity) : base(capacity, null) { }
    }

    public class FastLongDictionary<TValue> : FastDictionary<long, TValue>
    {

        public FastLongDictionary() : base(0, null) { }

        public FastLongDictionary(int capacity) : base(capacity, null) { }
    }
    ...
    public static class DictionaryExtensions
    {
        ...
        public static int FastFindEntry<TValue>(this FastDictionary<int, TValue> dict, int key)
        {
            if (dict.buckets != null)
            {
                //int hashCode = dict.comparer.GetHashCode(key) & 0x7FFFFFFF;
                int hashCode = key & 0x7FFFFFFF;
                for (int i = dict.buckets[hashCode % dict.buckets.Length]; i >= 0; i = dict.entries[i].next)
                {
                    if (dict.entries[i].hashCode == hashCode && (dict.entries[i].key) == key) return i;
                }
            }
            return -1;
        }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static int FastFindEntry<TValue>(this FastDictionary<long, TValue> dict, long key)
        {
            if (dict.buckets != null)
            {
                //int hashCode = dict.comparer.GetHashCode(key) & 0x7FFFFFFF;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static bool FastTryGetValue<TValue>(this FastDictionary<int, TValue> dict, int key, out TValue value)
        {
            int i = dict.FastFindEntry(key);
            if (i >= 0)
            {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static bool FastTryGetValue<TValue>(this FastDictionary<long, TValue> dict, long key, out TValue value)
        {
            int i = dict.FastFindEntry(key);
            if (i >= 0)
            {
    ...
    [Serializable]
    [ComVisible(false)]
    public class FastDictionary<TKey, TValue> : IDictionary<TKey, TValue>, IDictionary, IReadOnlyDictionary<TKey, TValue>, ISerializable, IDeserializationCallback
    {
