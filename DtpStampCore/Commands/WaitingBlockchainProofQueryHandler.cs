...
namespace DtpStampCore.Commands
{
    public class WaitingBlockchainProofQueryHandler : IRequestHandler<WaitingBlockchainProofQuery, IEnumerable<BlockchainProof>>
    {
        ...
        {
            var proofs = (request.IncludeTimestamps) 
                ? _db.Proofs.Include(p => p.Timestamps) 
                : _db.Proofs.AsQueryable();
            proofs = proofs.Where(p => p.Status == ProofStatusType.Waiting.ToString())
                        .Select(p => p);
